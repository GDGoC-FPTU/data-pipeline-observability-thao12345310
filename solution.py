...
==============================================================
Student ID: 2A202600049  (<-- Thay XXXX bang ma so cua ban)
...
def validate(data):
    ...
    print(f"{len(valid_records)} records kept, {error_count} dropped.")
...
def transform(data):
    ...
    df['category'] = df['category'].str.title()
    df['processed_at'] = datetime.datetime.now().isoformat()
    print(f"Transformed {len(df)} records. Added discounted_price and processed_at.")
    return df
